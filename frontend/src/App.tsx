...
function DashboardPage({ role }: { role: Role }) {
  ...
  const recent = data.recent || data.latestBookings || [];
  const detailBase = role === 'penanggung_jawab' ? '/pj/bookings' : role === 'admin' ? '/admin/bookings' : '/bookings';
  return <><PageTitle title={role === 'admin' ? 'Dashboard Admin' : role === 'penanggung_jawab' ? 'Dashboard Penanggung Jawab' : 'Dashboard Mahasiswa'} subtitle="Ringkasan aktivitas sistem"/><div className="grid gap-4 md:grid-cols-4">{Object.entries(stats).map(([k,v])=><StatCard key={k} label={k.replaceAll('_',' ')} value={v}/>)}</div><Card className="mt-6"><h3 className="mb-3 font-bold">Aktivitas Terbaru</h3>{recent.length ? <SimpleBookingTable items={recent} detailBase={detailBase}/> : <Empty text={role === 'penanggung_jawab' ? 'Belum ada aktivitas untuk area Anda. Pastikan akun PJ sudah terhubung ke area fasilitas yang benar.' : 'Belum ada aktivitas'}/>}</Card></>;
}
function SimpleBookingTable({ items, detailBase }: { items:any[]; detailBase?: string }) { return <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-gray-500"><th className="p-2">Fasilitas</th><th className="p-2">Tanggal</th><th className="p-2">Status</th><th className="p-2">Payment</th>{detailBase && <th className="p-2">Aksi</th>}</tr></thead><tbody>{items.map(b=><tr className="border-t" key={b.id}><td className="p-2 font-semibold">{b.facility?.name}</td><td className="p-2">{fmtDate(b.date)}</td><td className="p-2"><Badge value={b.status}/></td><td className="p-2"><Badge value={b.payment?.status}/></td>{detailBase && <td className="p-2"><Link to={`${detailBase}/${b.id}`}><Button variant="secondary">Detail</Button></Link></td>}</tr>)}</tbody></table></div>; }
...
function BookingsPage({ pj=false, history=false, admin=false }: any) {
  const [items,setItems]=useState<any[]>([]); const [error,setError]=useState(''); const url = pj ? `/pj/bookings${history ? '' : '?status=pending'}` : '/bookings';
  useEffect(()=>{ setError(''); apiGet<any[]>(url).then(setItems).catch(e=>setError(errMsg(e))); },[url]);
  const base = pj ? '/pj/bookings' : admin ? '/admin/bookings' : '/bookings';
  const emptyText = pj && !history ? 'Tidak ada booking pending untuk area Anda. Pastikan akun PJ sudah terhubung ke area fasilitas yang benar.' : 'Belum ada data';
  return <><PageTitle title={pj ? (history ? 'Riwayat Approval' : 'Booking Pending') : admin ? 'Semua Booking' : 'Peminjaman Saya'} subtitle="Data peminjaman fasilitas"/><Alert text={error}/>{!error && (items.length ? <Card><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="text-left text-gray-500"><th className="p-2">Fasilitas</th><th className="p-2">Mahasiswa</th><th className="p-2">Tanggal</th><th className="p-2">Status</th><th className="p-2">Payment</th><th className="p-2">Aksi</th></tr></thead><tbody>{items.map(b=><tr className="border-t" key={b.id}><td className="p-2 font-semibold">{b.facility?.name}</td><td className="p-2">{b.user?.name}</td><td className="p-2">{fmtDate(b.date)} {b.startTime}-{b.endTime}</td><td className="p-2"><Badge value={b.status}/></td><td className="p-2"><Badge value={b.payment?.status}/></td><td className="p-2"><Link to={`${base}/${b.id}`}><Button variant="secondary">Detail</Button></Link></td></tr>)}</tbody></table></div></Card> : <Empty text={emptyText}/>)}</>;
}
